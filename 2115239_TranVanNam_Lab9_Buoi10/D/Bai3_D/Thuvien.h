...
//Huy mot nut co khoa cho truoc ra khoi cay : nut the mang la phan tu nho nhat c? cay cay con phai
//Input : x, root
	...
	if (root != NULL)
	{
		if (strcmp(x.maNV, root->info.maNV) < 0)
			...
			if (strcmp(x.maNV, root->info.maNV) > 0)
				...
				if ((root->left == NULL) && (root->right == NULL)) //khong co con trai, khong co conphai
					root = NULL;
				else
